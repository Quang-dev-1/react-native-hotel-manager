...
export interface Room {
  ...
  _links?: {
    ...
    update?: { href: string };
    delete?: { href: string };
    updateStatus?: { href: string };
    rooms?: { href: string };
    sameStatus?: { href: string };
    sameFloor?: { href: string };
  };
}
...
const extractRooms = (response: any): Room[] => {
...
const extractRoom = (response: any): Room => {
  return response;
};
...
class RoomService {
  ...
  async getRoomById(id: number): Promise<Room> {
    try {
      ...
      return extractRoom(response.data);
    } catch (error: any) {
  ...
  async getRoomsByStatus(status: string): Promise<Room[]> {
    try {
      const response = await apiClient.get<any>(`/rooms/status/${status}`);
  ...
  async getRoomsByFloor(floor: number): Promise<Room[]> {
    try {
      const response = await apiClient.get<any>(`/rooms/floor/${floor}`);
      return extractRooms(response.data);
    } catch (error: any) {
  ...
  canDelete(room: Room): boolean {
    return !!room._links?.delete;
  }

  canUpdate(room: Room): boolean {
    return !!room._links?.update;
  }

  getSameStatusLink(room: Room): string | undefined {
    return room._links?.sameStatus?.href;
  }

  getSameFloorLink(room: Room): string | undefined {
    return room._links?.sameFloor?.href;
  }
}
